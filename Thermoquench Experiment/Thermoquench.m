...
for i = 1:length(data)
    ...
    [T_0,T_final,t,T_t] = trueStartGraph(t,T_t,data{i},experimentLinearPartsCount(i)); %Resets the graph so that it starts when the quench does
    t;
...
function [initTemp,finalTemp,newTime,newTemps] = trueStartGraph(time,Temp,dataTable,linearPartCount)
    ...
    startingInd = breakIndecies(linearPartCount) - 1; %starting temp is right before the break, since it cuts it off when it stops being linear
    startingTime = time(startingInd)
    
    initTemp = dataTable{startingInd,3};
    finalTemp = dataTable{end,3};
    
    time(1:startingInd-1) = []; %removing all indecies before the quench even starts, but not the starting time itself
