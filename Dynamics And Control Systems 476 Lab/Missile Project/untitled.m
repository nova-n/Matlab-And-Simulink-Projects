...
[beta_vs_time(:,1),beta_vs_time(:,2)+10^-3]
[beta_Tau,beta_settlingTime] = TwoPercentSettlingTimeTau([beta_vs_time(:,1),beta_vs_time(:,2)+10^-3],2,betaDesired+10^-3,10^-4,0.15)
[betaMax,betaPeakTime,betaMaxOvershoot,betaMaxPO] = overshootFinder([beta_vs_time(:,1),beta_vs_time(:,2)+10^-3],2,betaDesired+10^-3)
...
xline(beta_settlingTime,"-.",'LineWidth',1)
hold off;
xAxisLabel = "$Time \ (s)$";
xlabel(xAxisLabel,'Interpreter','latex') 
yAxisLabel = "$Angle \ of \ Side Step, \ \beta  \ ( deg^{\circ} ) $";
ylabel(yAxisLabel,'Interpreter','latex') 
legendStuff = ["$\beta$",strcat("$\beta_{desired} =" ,num2str(betaDesired) , " ^{\circ} $"),strcat("$P.O = ",num2str(betaMaxPO) ," \% $"),...
    strcat("$T_{p} = ", num2str(betaPeakTime) ," s $"),strcat("$T_{s} = " ,num2str(beta_settlingTime) , " s $")];
lgnd= legend(legendStuff,'Location','best');
set(lgnd, 'Interpreter','latex')
plot1 = "beta";
print('-r600','-dpng',plot1);
