...
ThreeVData = NaNRemover(ThreeVData);
FourVData = NaNRemover(FourVData);
FiveVData = NaNRemover(FiveVData);
...
ThreeV_fvVel = finalValueFinder(ThreeVData,[1/3,1]); % Smoothed Out Data, Tolerance, What Last fraction of graph you want, so 3 would be last 1/3
FourV_fvVel = finalValueFinder(FourVData,[0.65,0.8]);
FiveV_fvVel = finalValueFinder(FiveVData,[0.5,1]);

ThreeV_TimeConst = timeConstantFinder(ThreeVData,ThreeV_fvVel,0.06,0.2,"$3 V \ Dimnensionless \ Velocity$","$Dimnensionless \ Velocity$","3V")
FourV_TimeConst = timeConstantFinder(FourVData,FourV_fvVel,0.075,0.35,"$4 V \ Dimnensionless \ Velocity$","$Dimnensionless \ Velocity$","4V")
FiveV_TimeConst = timeConstantFinder(FiveVData,FiveV_fvVel,0.075,0.35,"$5 V \ Dimnensionless \ Velocity$","$Dimnensionless \ Velocity$","5V")

[ThreeV_FirstOrderApprox,ThreeV_Shift] = firstOrderApproximation(ThreeVData,ThreeVData,ThreeV_fvVel,ThreeV_TimeConst);
[FourV_FirstOrderApprox,FourV_Shift] = firstOrderApproximation(FourVData,FourVData,FourV_fvVel,FourV_TimeConst);
[FiveV_FirstOrderApprox,FiveV_Shift] = firstOrderApproximation(FiveVData,FiveVData,FiveV_fvVel,FiveV_TimeConst);
...
ThreeV_TwoPercent_Tau =TwoPercentSettlingTimeTau(ThreeVData(:,1:2),ThreeV_fvVel,1e-2,0.03)
FourV_TwoPercent_Tau =TwoPercentSettlingTimeTau(FourVData(:,1:2),FourV_fvVel,1e-2,0.03)
FiveV_TwoPercent_Tau =TwoPercentSettlingTimeTau(FiveVData(:,1:2),FiveV_fvVel,1e-2,0.03)
...
function [finalValue] = finalValueFinder(dataInputted,timeSection)
    ...
    finalValue = mean(dataInputted(lastSectionOfTime(1):lastSectionOfTime(2),2))
...
function [timeConstant] = timeConstantFinder(dataIn,finalVelocity,firstSectionPercentage,endSectionPercentage,givenTitle,measuredQuantity,voltageString)
    ...
    plot1 = strcat("RAW DATA Dimensionless Velocity And Time Constant For ", voltageString ,  " vs time");
...
function rawAndFilteredPlotter(originalData,filteredData,givenTitle,measuredQuantity,finalVal,timeConst,firstOrder,timeShift,voltageString)
    ...
    lgnd= legend(legendStuff,'Location','best');
    set(lgnd, 'Interpreter','latex')
    plot1 = strcat("RAW DATA All Velocity Plots For", voltageString ,  " vs time");
    print('-r600','-dpng',plot1);
end
...
function multiPlotter(datas,givenTitle,legendItems,measuredQuantity,fileNameString)
    ...
    set(lgnd, 'Interpreter','latex')
    plot1 = strcat("RAW DATA Multiplot Of", fileNameString ,  " vs time");
    print('-r600','-dpng',plot1);
end
...
function [settlingTimeVersion_Tau] = TwoPercentSettlingTimeTau(dataIn,finalVal,tol,streakFraction)
    ...
    chosenIndecies = find(abs(finalVal*0.98 - dataIn(:,2)) <= tol); %if the data is 98% of the final val within a tolerance
