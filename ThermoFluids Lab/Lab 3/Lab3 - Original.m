...
frictionFacs = c(1)+c(2)*Re.^c(3);


legendStuff = ["$Dark \ Blue \ Pipe$","$Light \ Blue \ Pipe$","$Best \ Fit \ Line$"];
figure;
plot(Re_DB,frictionFactorsDB);
hold on;
plot(Re_LB,frictionFactorsLB);
hold on;
plot(Re,frictionFacs,'k--');
hold off;
...
function [frictionFactorArray] = frictionFactorSolver(labSheet,diam,flowRates,sectionLength,manometerHeght1,manometerHeight2,grav)
    for i = 1:height(labSheet)
     frictionFactorArray(i) = (pi*grav * diam^4)/(8*flowRates(i).^2 * sectionLength) * (manometerHeight2(i) - manometerHeght1(i));   
    end
end
